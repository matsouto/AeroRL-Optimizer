...
class AirfoilScaler:
    ...
    def transform(self, weights, params):
        if self.w_min is None or self.w_max is None or self.p_min is None or self.p_max is None:
            raise RuntimeError("Scaler must be fitted before calling transform(). Call fit() first.")
    ...
    def inverse_transform(self, weights, params):
        ...
        """
        if self.w_min is None or self.w_max is None or self.p_min is None or self.p_max is None:
            raise RuntimeError("Scaler must be fitted before calling inverse_transform(). Call fit() first.")
